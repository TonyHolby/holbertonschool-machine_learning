...
class Yolo:
    ...
    def show_boxes(self, image, boxes, box_classes, box_scores, file_name):
        """
            ...
            Args:
                image (np.ndarray): the unprocessed image.
        ...
        """
        for boxe, classe, score in zip(boxes, box_classes, box_scores):
            x1, y1, x2, y2 = boxe.astype(int)
            cv2.rectangle(image,
                          (x1, y1),
                          (x2, y2),
                          color=(255, 0, 0),
            ...
            label = "{} {:.2f}".format(self.class_names[classe], score)

            position_text_x = x1
            if y1 - 5 > 10:
                position_text_y = y1 - 5
            else:
                position_text_y = y1 + 15

            cv2.putText(image,
                        label,
                        (position_text_x, position_text_y),
    ...
    def predict(self, folder_path):
        """
            Makes predictions on all images and displays them.
        ...
        for i, image in enumerate(images):
            model_outputs = self.model.predict(np.expand_dims(pimages[i],
                                                              axis=0))
            boxes, box_confidences, box_class_probs = self.process_outputs(
                model_outputs, image_shapes[i])
            filtered_boxes, box_classes, box_scores = self.filter_boxes(
                boxes, box_confidences, box_class_probs)
            box_predictions, pred_classes, pred_scores = \
            ...
            predictions.append((box_predictions, pred_classes, pred_scores))
