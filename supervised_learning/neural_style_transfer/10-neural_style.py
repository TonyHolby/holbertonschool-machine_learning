...
class NST:
    ...
    def __init__(self, style_image, content_image, alpha=1e4, beta=1, var=10):
        """
            Constructor for NST with variational cost.
    ...
    def total_cost(self, generated_image):
        """
            ...
            Returns:
                The total cost and individual components.
        """
    ...
    def compute_grads(self, generated_image):
        ...
        if generated_image.shape != expected_shape:
            raise TypeError(f"generated_image must be a tensor of shape "
                            f"{self.content_image.shape}")
    ...
    def variational_cost(generated_image):
        ...
        if generated_image.shape.rank != 4:
            raise TypeError(
                "generated_image must be a tensor of shape (1, nh, nw, 3)")

        x_deltas = tf.image.total_variation(generated_image)
        return tf.reduce_sum(x_deltas)
