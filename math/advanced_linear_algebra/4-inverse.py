#!/usr/bin/env python3
""" A script that calculates the inverse of a matrix. """
...
def adjugate(matrix):
    """
        ...
        Args:
            matrix (list): a list of lists whose adjugate matrix should be
            calculated.
    ...
    for row in zip(*cofactor_matrix):
        row_list = list(row)
        adjugate_matrix.append(row_list)
...
def inverse(matrix):
    """
        ...
        Args:
            matrix (list): a list of lists whose inverse should be calculated.
    ...
    for row in adjugate_matrix:
        inverse_row = []
        for element in row:
            inverse_row.append(element / determinant_matrix)
